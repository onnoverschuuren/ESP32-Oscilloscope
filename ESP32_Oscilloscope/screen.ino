...
void draw_sprite(float freq,
	...
	if (menu) {
		spr.setFreeFont(FM9);
		//spr.setTextSize(4);
		uint32_t selectkleur;
		selectkleur = TFT_RED;
		if (set_value) selectkleur = TFT_BLUE;
		...
		spr.setTextColor(TFT_WHITE, (opt == Sdiv) ? selectkleur : TFT_BLACK);
		spr.drawString(String(int(s_div)) + "uS/div", shift + 5, 33);
		spr.setTextColor(TFT_WHITE, (opt == Offset) ? selectkleur : TFT_BLACK);
		spr.drawString("Offset: " + String(offset) + "V", shift + 5, 48);
		spr.setTextColor(TFT_WHITE, (opt == TOffset) ? selectkleur : TFT_BLACK);
		spr.drawString("T-Off: " + String((uint32_t)toffset) + "uS", shift + 5, 63);
		spr.setTextColor(TFT_WHITE, (opt == Filter) ? selectkleur : TFT_BLACK);
		spr.drawString("Filter: " + str_filter, shift + 5, 78);
		spr.setTextColor(TFT_WHITE, (opt == Stop) ? selectkleur : TFT_BLACK);
		spr.drawString(str_stop, shift + 5, 93);
		spr.setTextColor(TFT_WHITE, (opt == Mode) ? selectkleur : TFT_BLACK);
		spr.drawString(wave_option, shift + 5, 108);
		spr.setTextColor(TFT_WHITE, (opt == Single) ? selectkleur : TFT_BLACK);
		spr.drawString("Single " + String(single_trigger ? "ON" : "OFF"), shift + 5, 123);
		spr.setTextColor(TFT_WHITE, (opt == Reset) ? selectkleur : TFT_BLACK);
...
void draw_grid() {
	uint32_t pixelkleur;
	pixelkleur = TFT_DARKGREEN;
	for (int i = 0; i < 32; i++) {
		spr.drawPixel(i * 10, 0, pixelkleur);
		spr.drawPixel(i * 10, 40, pixelkleur);
		spr.drawPixel(i * 10, 80, pixelkleur);
		spr.drawPixel(i * 10, 120, pixelkleur);
		spr.drawPixel(i * 10, 160, pixelkleur);
		spr.drawPixel(i * 10, 200, pixelkleur);
		spr.drawPixel(i * 10, 239, pixelkleur);
	}
	for (int i = 0; i < 240; i += 10) {
		for (int j = 0; j < 320; j += 40) {
			spr.drawPixel(j, i, pixelkleur);
		}
	}
	for (int i = 0; i < 240; i += 10) {
		spr.drawPixel(319, i, pixelkleur);
	}
	spr.drawPixel(319, 239, pixelkleur);
}
